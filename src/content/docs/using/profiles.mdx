...
<Callout type="note">
  Wide preformatted blocks are scaled to fit the column rather than overflowing the page, so a wide ASCII banner
  degrades gracefully on a phone.
</Callout>
...
<Callout type="warning">
  There is no undo, and no export-then-delete flow that guarantees your posts come back. Save anything you want to keep
  first.
</Callout>
