...
<Callout type="warning" title="Backend imports must be type-only">
  The `@` alias resolves to `apps/backend/src`, and every import through it must be `import type`. The backend is Deno;
  its modules cannot be resolved at runtime here. Type-only imports are erased at build time (`verbatimModuleSyntax`),
  which is what keeps backend code out of the bundle — and what lets the frontend image build from `apps/frontend`
  alone, with no `apps/backend` in its context.
</Callout>
...
<Callout type="danger" title="No default-palette colours">
  `text-neutral-*`, `bg-gray-*`, `text-red-600`, and raw `bg-white` are all violations. Use `text-foreground`,
  `bg-muted`, `text-destructive`, `bg-background`.
</Callout>
