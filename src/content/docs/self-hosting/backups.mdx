...
| `backups`    | Automatic restic snapshots (above)               | N/A — this _is_ the backup            |
...
<Callout type="warning" title="The pairing rule">
  `secrets/db_password` is bound to whatever `pgdata` was created with. The database and that password must always
  travel **together** — or neither. A partial mix (new `pgdata` with old `secrets`) breaks the pairing and the backend
  will not connect.
</Callout>
...
— so `secrets` is _not_ backed up — which means everyone is signed out once.
...
<Callout type="danger" title="Store backups privately">
  Automatic snapshots are encrypted at rest, but the manual files below are **not**: Method B tarballs contain your
  database password and session secret, and Method A dumps contain every account's data. Encrypt them, and keep every
