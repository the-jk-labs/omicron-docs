...
If the tags are missing and the response is small, check the title: if it is
*"Making sure you're not a bot!"*, the [scraper shield](/self-hosting/security/)
answered instead of the article. That should not happen — post URLs are not
challenged — so look at whether `botPolicy.yaml` has been customised. If the tags are there,
